      ...
      @foreach ($req as $val)
        ...
        @endif
            <div class='mt-3 d-flex justify-content-start p-2'>
              <p class='h6 m-0'><span class='text-secondary'>Requested To : </span>{{$val->donor->name->first_name}} {{$val->donor->name->last_name}}</p>
            </div>
            @if($val->status=='Pending')
              <p class='h6 m-0 p-2'><span class='text-secondary'>Status : </span>{{$val->status}}</p>
            @elseif($val->status=='Denied')
              <p class='h6 m-0 p-2'><span class='text-secondary'>Status : </span><span class='bg-danger p-1'>{{$val->status}}</span></p>
            @else
              <p class='h6 m-0 p-2'><span class='text-secondary'>Status : </span><span class='bg-success p-1'>{{$val->status}}</span></p>
            @endif
